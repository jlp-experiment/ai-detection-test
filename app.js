...
import { ChevronRight, BookOpen, Check, AlertCircle } from 'lucide-react';

const FictionDetectionTest = () => {
  const [stage, setStage] = useState('welcome');
  const [group, setGroup] = useState(null); // 'control' or 'experimental'
  ...
  const [primingSamples, setPrimingSamples] = useState([]); // For experimental group
  const [testSamples, setTestSamples] = useState([]); // The 3 random samples to rate
  const [currentSampleIndex, setCurrentSampleIndex] = useState(0);
  const [currentStage, setCurrentStage] = useState('priming'); // 'priming' or 'testing'
    ...
    const initFirebase = async () => {
      try {
        ...
        const firebaseConfig = {
          apiKey: "AIzaSyC03LDNtcFOEPqvPK21GWvoMfVTc7RnJfA",
          authDomain: "ai-fiction-detection-project.firebaseapp.com",
          projectId: "ai-fiction-detection-project",
          storageBucket: "ai-fiction-detection-project.firebasestorage.app",
          messagingSenderId: "928161649455",
          appId: "1:928161649455:web:c096c556c13bbe28eb47da"
        };
  ...
  useEffect(() => {
    const loadSamples = async () => {
      ...
      try {
        ...
        if (assignedGroup === 'experimental') {
          // Get priming samples (id 1 and id 12)
          const sample1 = data.find(s => s.id === 1);
          const sample12 = data.find(s => s.id === 12);
          setPrimingSamples([sample1, sample12]);
          
          // Get 3 random samples excluding id 1 and 12, no family duplicates
          const availableSamples = data.filter(s => s.id !== 1 && s.id !== 12);
          const selectedSamples = selectSamplesNoFamilyDuplicates(availableSamples, 3);
          setTestSamples(selectedSamples);
          setCurrentStage('priming');
        } else {
          // Control group: just get 3 random samples, no family duplicates
          const selectedSamples = selectSamplesNoFamilyDuplicates(data, 3);
          setTestSamples(selectedSamples);
  ...
  // Helper function to select samples without family duplicates
  ...
  const saveResults = async () => {
    ...
    try {
      ...
      const resultData = {
        ...
        group: group, // 'control' or 'experimental'
        demographics: demographics,
        primingSamples: group === 'experimental' ? primingSamples.map(s => ({
          id: s.id,
          familyId: s.familyId,
          description: s.description,
          isAI: s.isAI
        })) : [],
        testSamples: testSamples.map(s => ({
          id: s.id,
          familyId: s.familyId,
          description: s.description,
          isAI: s.isAI
        ...
        favoriteSample: testSamples.find(s => s.id === favoriteId),
        accuracy: correctCount / testSamples.length,
        correctCount: correctCount,
        totalSamples: testSamples.length
      };

      await addDoc(collection(window.db, 'test_results'), resultData);
      setSaving(false);
    } catch (err) {
  ...
  const canStartTest = demographics.age && demographics.gender && demographics.isProfessional && demographics.readingVolume;
  ...
  const handlePrimingNext = () => {
    if (currentSampleIndex < primingSamples.length - 1) {
  ...
  const handleTestingNext = () => {
    ...
    // Move to next sample or favorite selection
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 md:p-8">
        <div className="max-w-3xl mx-auto bg-white rounded-lg shadow-xl p-6 md:p-8">
          ...
          <div className="border-t pt-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Before we begin:</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Are you a professional writer, editor, teacher, critic, or someone who 
                  professionally critiques writing quality?
                </label>
                <div className="space-y-2">
                  <label className="flex items-center">
                    <input
                      ...
                      name="professional"
                      value="yes"
                      checked={demographics.isProfessional === 'yes'}
                      onChange={(e) => handleDemographicChange('isProfessional', e.target.value)}
                      className="mr-2"
                    />
                    Yes
                  </label>
                  <label className="flex items-center">
                    <input
                      ...
                      name="professional"
                      value="no"
                      checked={demographics.isProfessional === 'no'}
                      onChange={(e) => handleDemographicChange('isProfessional', e.target.value)}
                      className="mr-2"
                    />
                    No
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  How much fiction do you read per month? (books, comics, short stories, etc.)
                </label>
                <div className="space-y-2">
                  <label className="flex items-center">
                    <input
                      ...
                      name="reading"
                      value="0-2"
                      checked={demographics.readingVolume === '0-2'}
                      onChange={(e) => handleDemographicChange('readingVolume', e.target.value)}
                      className="mr-2"
                    />
                    0-2 books.
                  </label>
                  <label className="flex items-center">
                    <input
                      ...
                      name="reading"
                      value="3-5"
                      checked={demographics.readingVolume === '3-5'}
                      onChange={(e) => handleDemographicChange('readingVolume', e.target.value)}
                      className="mr-2"
                    />
                    3-5 books.
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      name="reading"
                      value="6+"
                      checked={demographics.readingVolume === '6+'}
                      onChange={(e) => handleDemographicChange('readingVolume', e.target.value)}
                      className="mr-2"
                    />
                    6+ books.
            ...
            <button
              ...
              className={`mt-6 w-full flex items-center justify-center gap-2 px-6 py-3 rounded-md text-white font-medium ${
                canStartTest
  ...
  // Priming screen (experimental group only)
  if (stage === 'testing' && group === 'experimental' && currentStage === 'priming') {
    const currentSample = primingSamples[currentSampleIndex];
    const isPrimingSample1 = currentSampleIndex === 0;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 md:p-8">
        <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-xl p-6 md:p-8">
          <div className="mb-6">
            <div className="bg-indigo-100 border border-indigo-300 rounded-lg p-4 mb-4">
              <p className="text-indigo-900 font-medium">
                This sample is <strong>{isPrimingSample1 ? 'human-written' : 'AI-generated'}</strong>. 
                Please read it carefully.
              </p>
            </div>
          </div>

          <div className="prose prose-lg max-w-none mb-8 p-6 bg-gray-50 rounded-lg max-h-96 overflow-y-auto">
          ...
          >
            {currentSampleIndex < primingSamples.length - 1 ? 'Next Example' : 'Begin Rating Samples'}
            <ChevronRight className="w-5 h-5" />
          </button>
        </div>
      </div>
  ...
  // Testing screen (both groups rate samples here)
  if (stage === 'testing' && currentStage === 'testing' && testSamples.length > 0) {
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-sm text-gray-600">
                Progress: {Math.round(((currentSampleIndex + 1) / testSamples.length) * 100)}%
              </div>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className="bg-indigo-600 h-2 rounded-full transition-all"
                style={{ width: `${((currentSampleIndex + 1) / testSamples.length) * 100}%` }}
              ></div>
            </div>
          </div>
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={`mt-1 w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                    ...
                    {favoriteId === sample.id && <Check className="w-3 h-3 text-white" />}
                  </div>
                  <div>
